...
contract SliceMetaTxn is ERC2771Context, Ownable {
    ...
    function setupSlice(
        ...
        require(
            _recipientAddresschainId != uint(0),
            "Recipient address chain required"
        );
    ...
    function getSliceInfo()
        external
        view
        returns (address, address, address, uint, uint)
    {
        return (
            exchangeAddress,
            targetToken,
    ...
    function getPayer(
        uint _index
    ) external view returns (bytes memory, uint, uint) {
    ...
    function _addPayer(bytes memory _payeruid, uint _amountDue) private {
    ...
    function addPayer(
        bytes memory _payeruid,
        uint _amountDue
    ) internal onlyOwner notPaused {
    ...
    function swapTo(
        ...
        uint tokenUnspent = _amountInputMaximum - amounts[0];
